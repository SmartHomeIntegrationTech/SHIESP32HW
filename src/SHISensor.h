...
struct SensorData {
    SensorData(SensorDataType type, std::function<String(SensorData&)> toTransmitString, String name):
        type(type), 
...
struct SensorReadings {
    SensorReadings(unsigned long timeStamp, std::vector<std::shared_ptr<SHI::SensorData>> data):
        timeStamp(timeStamp), 
        data(data) 
        {}
